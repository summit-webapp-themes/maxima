...
const CancelOrder = ({orderHistoryItems}:any) => {
  const dispatch = useDispatch();

  const orderCancelled = orderHistoryItems.length > 0 && orderHistoryItems.filter((items:any)=>(items?.payment_status ==="Cancelled"))
            ...
            <div className="row">
              ...
              <div className="col text-end">
                <p className="mb-0" style={{ fontSize: "15px" }}>
                  <span className="bold">{orderCancelled?.length}</span> orders
                </p>
              </div>
            </div>
          </div>
